...
def summarize(*args):
    ...
    _sum = sum(args)
    evenList = [i for i in args if i % 2 == 0]
    oddList = [i for i in args if i % 2 != 0]
    return dict, _sum, evenList, oddList

print(summarize(1, 2, 3, 3, 4, 5, 6))

# Output: ({1: 1, 2: 1, 3: 2, 4: 1, 5: 1, 6: 1}, 24, [2, 4, 6], [1, 3, 3, 5])
...
def flatten_and_process(nested_list):
    ...
    return flat, numbers, strings, sum(numbers), "".join(strings)
...
def word_analysis(sentence):
    ...
    length_map = {len(w): [x for x in words if len(x) == len(w)] for w in unique_words}
    return long_words_count, unique_words, length_map
...
def sort_students(data):
    sorted_data = sorted(data, key=lambda x: (-x[2], x[1]))  # score desc, age asc
